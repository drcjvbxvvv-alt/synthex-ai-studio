...
## 誠實的理由：為什麼是 Project Brain，而不是更強的 LLM？

> 這不是推銷文。這是對一個合理問題的直接回答。

### 核心問題：LLM 越來越強，還需要這個嗎？

**是的，需要。但原因不是你想的那個。**

Claude Opus 4、GPT-4o、Gemini Ultra——它們在推理、理解、生成程式碼方面都已達到令人驚嘆的水準。問題從來不是 LLM 不夠聰明。
...
它不知道：
  × 你們六個月前已經踩過這個坑
  × 你們的 Stripe 整合有一個非標準的 idempotency_key 規範
  × payment_service 的事務邊界和其他服務不同
  × 上一個修這裡的人留下了一個隱性假設

這些都不在任何訓練數據裡。它們只存在於你的 git 歷史和你同事的腦子裡。
```
...
適合：
- **使用 AI Coding Agent（Claude Code、Cursor、Copilot）進行真實開發的團隊**
- 專案存在 6 個月以上、有累積的架構決策和踩坑記錄的程式碼庫
- 多人協作、知識在不同人腦中分散的團隊
- 每次重新開啟 AI 對話都要「重新解釋背景」的工程師

不適合：
- 全新專案、尚無歷史的程式碼庫（冷啟動期效益低）
- 純粹探索性、一次性的 AI 對話
- 期望「裝上就有效」、不願意主動 `brain add` 的使用者
...
即使你有 200k token 的上下文視窗，你也無法把六個月的決策歷史都塞進去。而且更大的問題是：你不知道「今天這個任務」需要的是哪 3 條規則。Brain 做的事是把 6 個月壓縮成「與今天任務相關的 2000 token」。

**問題 2：專案特有知識不在任何 LLM 的訓練數據裡。**

LLM 知道 Stripe 的通用最佳實踐。它不知道你的團隊決定用 `X-Idempotency-Key` 而不是 `Idempotency-Key`，因為你用的 SDK 版本有一個特定 bug。這種知識只存在於你的 commit e3f2a19 的訊息裡。

**問題 3：AI Agent 做出決策，但下次不記得。**

AI Agent 幫你重構了資料庫存取層、在 PR 裡解釋了為什麼選這個方案——然後對話結束了。下個月你開新對話，它再次面對同樣的問題，可能做出完全不同的選擇。Brain 在 Agent 每次 commit 時自動記錄這些決策。

**Brain 不解決的問題（誠實說）：**
- 它不讓 LLM 變得更聰明
- 它不替代你自己思考架構
- 它無法捕捉「不言而喻」的隱性知識——你沒有寫下來的，它也無從記錄
- 冷啟動期（前 2–4 週）效益有限，需要主動建立知識庫
...
```
第 1 週：幾乎沒有效益（知識庫為空）
第 1 個月：開始看到相關規則出現在上下文中
第 3 個月：Agent 自動避開你們踩過的坑
第 6 個月：新加入的 Agent（和新成員）能快速理解專案脈絡
```

特別有效的場景：
- 「這個功能上次怎麼做的？」類型的查詢
- 跨越 LLM 對話邊界的知識延續
- 新成員 onboarding（讓 Agent 成為帶新人的「老員工」）
- 技術債整理（哪些決策是有意為之，哪些是歷史包袱）
...
```
你說任務 → Agent 先查 Brain → Agent 帶著專案上下文回應
                                ↑
                           這 2 秒，是差異所在
```

不是在 LLM 推理的過程中，而是在**推理開始之前的上下文注入**。Brain 本身不推理，它提供素材讓 LLM 推理得更準。
...
| 方案 | 問題 |
|------|------|
| 把所有文件放進 context | 無法 fit；不知道哪些相關；每次要手動選 |
| 用 Notion / Confluence | Agent 不會主動查；需要手動整理 |
| 依賴 git log | 非結構化；LLM 每次重新解析，成本高 |
| 用通用記憶工具（Mem0 等）| 無工程特化 schema；無版本衰減；依賴外部服務 |
| 什麼都不做 | 依賴工程師記憶；知識隨人員流動而流失 |

Brain 的定位是：**針對工程程式碼庫的結構化長期記憶，零外部依賴，與 git 工作流原生整合。**
...
工作流程：
```
1. brain setup（5 分鐘，一次性）
2. 每次 git commit 自動記錄（無摩擦）
3. 遇到重要決策，brain add（30 秒）
4. Agent 每次任務開始自動查詢（對 Agent 透明）
```

衡量效果的方式（誠實的）：
- 「Agent 提到了我上次的決策」的頻率
- 「這個坑以前踩過，這次 Agent 自動避開了」的次數
...
**LLM 解決「如何做」的問題。Project Brain 解決「在這個專案裡，我們決定怎麼做」的問題。兩者不競爭，Brain 讓 LLM 的推理更有專案脈絡。**
...
輸出中的信心值附帶語意標記，讓 Agent 立刻判斷可信程度：

| 標注 | 範圍 | 意義 |
|------|------|------|
| `⚠ 推測` | 0.0–0.3 | 來自低品質 commit，謹慎使用 |
| `~ 推斷` | 0.3–0.6 | 自動提取，可能正確 |
| `✓ 已驗證` | 0.6–0.8 | 人工加入或多次查詢確認 |
| `✓✓ 權威` | 0.8–1.0 | KRB 審核通過或高信心手動加入 |
...
| `BRAIN_EXPAND_LIMIT` | `15`                        | 查詢展開詞彙上限（減少雜訊）     |
