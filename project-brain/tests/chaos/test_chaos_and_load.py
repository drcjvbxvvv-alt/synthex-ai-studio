...
class TestV81InterviewNonInteractive:
    ...
    def test_dispatch_audit(self, tmp_path):
        """核心命令在 project_brain/cli.py dispatch（v10.2）"""
        content = open('project_brain/cli.py').read()
        # Only 10 core commands remain after CLI cleanup
        assert "'setup':" in content or "'ask':" in content

    def test_answers_argparse(self, tmp_path):
        """brain setup --help 可正常執行（interview 命令已移除）"""
        import subprocess
        r = subprocess.run(['python', 'brain.py', 'setup', '--help'],
