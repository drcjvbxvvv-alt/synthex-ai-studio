...
> **當前版本**：v0.22.0（2026-04-05）
> **文件用途**：待辦改善項目。已完成項目見 `CHANGELOG.md`。
> **分析基準**：877 tests collected；v0.21.0 全部 P1+P2+P3 完成，855 passed / 5 skipped。
...
| 優先 | ID | 影響摘要 | 象限 | 狀態 |
|------|----|---------|------|------|
| **P2** | MEM-01 | AI 輔助相關性選取（召回精準度大幅提升） | 🎯 高價值 | 📋 待實作 |
| **P2** | MEM-02 | `description` 欄位 + 摘要/全文分層載入 | 🎯 高價值 | 📋 待實作 |
| **P2** | MEM-03 | `alreadySurfaced` session 內去重 | ⚡ 快速獲益 | 📋 待實作 |
| **P2** | MEM-04 | 過時節點明確警告文字注入 context | ⚡ 快速獲益 | 📋 待實作 |
| **P3** | MEM-05 | `recentTools` 降權（減少 context 雜訊） | 🔵 填空 | 📋 待實作 |
| **P3** | MEM-06 | 摘要層 / 詳細層 context 分離 | 🏗 長期 | 📋 待實作 |
...
```
MEM-02 (description 欄位) ──→ MEM-01 (AI 選取依賴 description 做輸入)
MEM-03 ──→ 無依賴（MCP session state）
MEM-04 ──→ 無依賴（get_context 輸出後處理）
MEM-05 ──→ MEM-03（同屬 get_context 呼叫端過濾邏輯）
MEM-06 ──→ MEM-02（摘要層使用 description 欄位）

TEST-04 ──→ 無依賴（等功能穩定）
FEAT-08 ──→ 無依賴（維持擱置）
```

**建議實作順序**：MEM-04 → MEM-03 → MEM-02 → MEM-01 → MEM-05 → MEM-06
...
`get_context` 加可選的 `ai_select=True` 模式：

```python
# engine.py get_context()
def get_context(self, task: str, ai_select: bool = False) -> str:
    # Stage 1: 機械式召回前 20 個候選
...
| 指標 | 預期改善 |
|------|---------|
| 語意相關召回率 | FTS/ANN 約 70% → AI 選取約 90%+ |
| context token 用量 | 不變（仍取最多 5 個節點） |
| 延遲 | 增加 1 次 LLM side query（~500ms） |

#### 驗收條件

- `brain context "設計 API 驗證流程" --ai-select` 能召回 `hmac.compare_digest` 相關 Pitfall
- `test_engine.py` 新增 `test_ai_select_retrieves_semantic_match`

---

### MEM-02 — `description` 欄位 + 索引/召回分離
...
### MEM-03 — `alreadySurfaced` Session 內去重
...
```bash
pytest tests/ -x -q
# 目標：≥ 880 passed，0 failed
```

| 項目 | 驗收條件 |
|------|---------|
| MEM-04 | `test_engine.py` 60 天前節點的 context 含警告文字 |
| MEM-03 | `test_mcp.py` 同 session 兩次 `get_context` 不重複回傳相同節點 |
| MEM-02 | `brain add --description` 寫入 DB；`get_node` 回傳 `description` 欄位 |
| MEM-01 | `brain context --ai-select` 能召回語意相關但關鍵字不重疊的 Pitfall |
| MEM-05 | Rule 節點在 `current_context_tags` 重疊時排名下降；Pitfall 不受影響 |
| MEM-06 | `get_context(detail_level="summary")` 回傳 token 數 < `full` 模式的 30% |
