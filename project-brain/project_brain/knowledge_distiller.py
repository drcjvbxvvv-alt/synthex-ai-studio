...
功能：
  ...
  Layer 1 — Context Distillation（已實作）：
    把知識圖譜壓縮成結構化 Markdown 摘要（BRAIN_KNOWLEDGE.md）
...
class KnowledgeDistiller:
    ...
    def _distill_context(self, nodes: list[dict]) -> Path:
        """
        壓縮整個知識圖譜為結構化 Markdown（BRAIN_KNOWLEDGE.md）。
        ...
        content = "\n".join(lines)
        path    = self.out_dir / "BRAIN_KNOWLEDGE.md"
    ...
    def _distill_role_prompts(self, nodes: list[dict]) -> list[Path]:
        """
        為每個 AI Agent 角色生成針對性的 system prompt 片段。
