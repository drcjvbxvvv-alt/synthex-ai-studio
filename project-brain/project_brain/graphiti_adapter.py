...
class KnowledgeEpisode:
    ...
    """
    content:      str
    source:       str = "project-brain"      # 來源（agent 名稱、commit、ship）
...
def _run_async(coro, timeout: float = 10.0):
    """
    安全執行 async coroutine，兼容有/無 running event loop 的環境（v8.0）。

    問題背景：
    - 正常 CLI 環境：無 running loop → asyncio.run() 正常
    - Jupyter / FastAPI / Streamlit：已有 running loop → asyncio.run() 拋 RuntimeError

    解法：
    - 有 running loop → 在新執行緒建立獨立 event loop 並 Future 等待結果
    - 無 running loop → 直接 asyncio.run()

    取代 graphiti_adapter.py 中所有裸露的 asyncio.run()。
    """
    try:
        asyncio.get_running_loop()
        # 已有 running loop（Jupyter/FastAPI 等）
        ...
        # 無 running loop（一般 CLI 環境）
        return asyncio.run(coro)


class GraphitiAdapter:
    """
    Graphiti 時序知識圖譜的 Project Brain 適配層（L2）。
    ...
    def available(self) -> bool:
        """Graphiti 是否可用（懶初始化）
        B-2: GRAPHITI_DISABLED=1 時直接回傳 False，跳過連線嘗試
        """
        import os as _os
        if _os.environ.get('GRAPHITI_DISABLED', '0') == '1':
            return False  # local-only 模式：L2 完全跳過
    ...
    def _try_connect(self) -> bool:
        """A-12: graphiti_core imported lazily here — not at module load."""
        """
    ...
    def add_episode_sync(self, episode: KnowledgeEpisode) -> bool:
        ...
        try:
            try:
                ...
                return _run_async(self.add_episode(episode))
        except Exception as e:
    ...
    def search_sync(self, query: str, top_k: int = 5) -> list[TemporalSearchResult]:
        ...
        try:
            ...
            return _run_async(self.search(query, top_k))
        except Exception as e:
