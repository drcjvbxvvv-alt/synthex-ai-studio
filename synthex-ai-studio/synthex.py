...
def cmd_brain_scan(args):
    ...
    print(f"\n{GREEN}✓{RESET} 考古報告  {CYAN}{report_path}{RESET}")
    print(report[:1000])
...
def cmd_brain_context(args):
    ...
    if ctx:
        ...
        print(f"{YELLOW}⚠{RESET}  知識庫為空，請先執行：")
        print(f"  {DIM}python synthex.py brain scan --workdir .{RESET}")
...
def cmd_brain_query_shared(args):
    ...
    if not results:
        print(f"{YELLOW}⚠{RESET}  共享庫中無相關知識")
        return
    print(f"\n{CYAN}{BOLD}🌐  跨專案知識{RESET}  找到 {BOLD}{len(results)}{RESET} 筆\n{DIM}{'─'*50}{RESET}")
    for r in results:
        print(f"\n  {CYAN}[{r['type']}]{RESET} {BOLD}{r['title']}{RESET}")
        print(f"  {DIM}來源：{r['namespace']}  信心：{r['confidence']:.0%}{RESET}")
...
def cmd_brain_decay(args):
    ...
    if action == "report":
        ...
        result = de.run()
        print(f"{GREEN}✓{RESET} 衰減掃描完成，更新 {BOLD}{len(result)}{RESET} 筆知識")
