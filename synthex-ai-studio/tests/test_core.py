"""
SYNTHEX AI STUDIO — Unit Test Suite
...
# ══════════════════════════════════════════════════════════════
# ══════════════════════════════════════════════════════════════
# Test Group 16：第十二輪修復 — Beta Headers + Swarm agentic_mode
# ══════════════════════════════════════════════════════════════

class TestBetaHeaderConstants(unittest.TestCase):
    """P0-1/P0-2：Beta header 常數集中管理"""

    def test_beta_interleaved_thinking_is_string_or_none(self):
        """BETA_INTERLEAVED_THINKING 應為 str 或 None"""
        from core.base_agent import BETA_INTERLEAVED_THINKING
        self.assertIn(type(BETA_INTERLEAVED_THINKING), (str, type(None)),
                      "BETA_INTERLEAVED_THINKING 應為 str 或 None")

    def test_beta_context_management_is_string_or_none(self):
        """BETA_CONTEXT_MANAGEMENT 應為 str 或 None"""
        from core.base_agent import BETA_CONTEXT_MANAGEMENT
        self.assertIn(type(BETA_CONTEXT_MANAGEMENT), (str, type(None)),
                      "BETA_CONTEXT_MANAGEMENT 應為 str 或 None")

    def test_build_run_params_no_hardcoded_beta(self):
        """_build_run_params 不應包含硬編碼的 beta 字串"""
        import inspect
        from core import base_agent
        src = inspect.getsource(base_agent.BaseAgent._build_run_params)
        self.assertNotIn('"interleaved-thinking-2025-05-14"', src,
                         "不應直接硬編碼 beta header 字串，應使用常數")

    def test_context_management_params_no_hardcoded_beta(self):
        """build_context_management_params 不應包含硬編碼的 beta 字串"""
        import inspect
        from core import base_agent
        src = inspect.getsource(base_agent.CompactionManager.build_context_management_params)
        self.assertNotIn('"context-management-2025-06-27"', src,
                         "不應直接硬編碼 context-management beta，應使用常數")

    def test_build_run_params_uses_beta_constant(self):
        """_build_run_params 應引用 BETA_INTERLEAVED_THINKING"""
        import inspect
        from core import base_agent
        src = inspect.getsource(base_agent.BaseAgent._build_run_params)
        self.assertIn("BETA_INTERLEAVED_THINKING", src,
                      "_build_run_params 應使用 BETA_INTERLEAVED_THINKING 常數")

    def test_context_management_returns_dict(self):
        """build_context_management_params 應回傳 dict"""
        from core.base_agent import CompactionManager
        cm = CompactionManager(model="claude-opus-4-6", agent_name="TEST")
        result = cm.build_context_management_params()
        self.assertIsInstance(result, dict,
                              "build_context_management_params 應回傳 dict")
        self.assertIn("context_management", result,
                      "結果應包含 context_management 鍵")

    def test_context_management_params_conditional_betas(self):
        """BETA_CONTEXT_MANAGEMENT=None 時不應有 betas 鍵"""
        from core import base_agent
        original = base_agent.BETA_CONTEXT_MANAGEMENT
        try:
            base_agent.BETA_CONTEXT_MANAGEMENT = None
            from core.base_agent import CompactionManager
            cm = CompactionManager(model="claude-opus-4-6")
            result = cm.build_context_management_params()
            self.assertNotIn("betas", result,
                             "BETA=None 時結果不應有 betas 鍵")
        finally:
            base_agent.BETA_CONTEXT_MANAGEMENT = original


class TestSwarmAgenticMode(unittest.TestCase):
    """P1-1：SwarmTask.agentic_mode 旗標"""

    def test_swarm_task_has_agentic_mode(self):
        """SwarmTask 應有 agentic_mode 欄位"""
        from core.swarm import SwarmTask
        task = SwarmTask(task_id="t1", agent_name="ECHO", prompt="test")
        self.assertFalse(task.agentic_mode,
                         "預設 agentic_mode 應為 False")

    def test_swarm_task_agentic_mode_true(self):
        """SwarmTask 應接受 agentic_mode=True"""
        from core.swarm import SwarmTask
        task = SwarmTask(task_id="t1", agent_name="BYTE", prompt="實作功能",
                         agentic_mode=True)
        self.assertTrue(task.agentic_mode)

    def test_run_task_uses_agentic_mode(self):
        """_run_task 原始碼應檢查 task.agentic_mode"""
        import inspect
        from core.swarm import AgentSwarm
        src = inspect.getsource(AgentSwarm._run_task)
        self.assertIn("agentic_mode", src,
                      "_run_task 應根據 agentic_mode 選擇 chat() 或 run()")

    def test_run_task_calls_run_when_agentic(self):
        """_run_task 原始碼在 agentic_mode=True 時應呼叫 agent.run()"""
        import inspect
        from core.swarm import AgentSwarm
        src = inspect.getsource(AgentSwarm._run_task)
        self.assertIn("agent.run(", src,
                      "agentic_mode=True 時應呼叫 agent.run()")

    def test_run_task_calls_chat_when_not_agentic(self):
        """_run_task 原始碼在 agentic_mode=False 時應呼叫 agent.chat()"""
        import inspect
        from core.swarm import AgentSwarm
        src = inspect.getsource(AgentSwarm._run_task)
        self.assertIn("agent.chat(", src,
                      "agentic_mode=False 時應呼叫 agent.chat()")


class TestBannerVersion(unittest.TestCase):
    """P1-2：Banner 版本號"""

    def test_banner_shows_v3(self):
        """synthex.py BANNER 應顯示 v0.0.0"""
        import importlib.util
        spec = importlib.util.spec_from_file_location(
            "synthex_mod",
            str(__import__("pathlib").Path(__file__).parent.parent / "synthex.py")
        )
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
        self.assertIn("v0.0.0", mod.BANNER,
                      "BANNER 應顯示 v0.0.0")
        self.assertNotIn("v2", mod.BANNER,
                         "BANNER 不應仍顯示舊版本 v2")
        self.assertNotIn(" v3 ", mod.BANNER,
                         "BANNER 不應仍顯示舊版本 v3")
...
class TestMCPServer(unittest.TestCase):
    ...
    def test_mcp_server_exists(self):
        """core/mcp_server.py 應存在"""
        from pathlib import Path
        mcp_path = Path(__file__).parent.parent / "core" / "mcp_server.py"
        self.assertTrue(mcp_path.exists(),
                        "core/mcp_server.py 應存在")

    def test_mcp_server_importable(self):
        """core.mcp_server 應可 import"""
        try:
            import core.mcp_server as mcp
        except ImportError as e:
            self.fail(f"core.mcp_server import 失敗：{e}")

    def test_mcp_server_has_tools(self):
        """MCP server 應定義工具列表"""
        import core.mcp_server as mcp
        self.assertTrue(hasattr(mcp, "SYNTHEX_MCP_TOOLS"),
                        "mcp_server 應定義 SYNTHEX_MCP_TOOLS")

    def test_mcp_server_tool_names(self):
        """MCP 工具應包含核心操作"""
        import core.mcp_server as mcp
        tool_names = {t["name"] for t in mcp.SYNTHEX_MCP_TOOLS}
        required = {"synthex_ask", "synthex_agent", "synthex_list_agents"}
        missing = required - tool_names
        self.assertFalse(missing, f"MCP 工具缺少：{missing}")
