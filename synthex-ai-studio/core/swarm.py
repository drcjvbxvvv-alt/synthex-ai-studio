...
class SwarmTask:
    ...
    agentic_mode:   bool        = False  # True → agent.run()（帶工具），False → agent.chat()
...
class AgentSwarm:
    ...
    def _run_task(self, task: SwarmTask, context: str) -> str:
        ...
        模式：
          agentic_mode=False（預設）→ agent.chat()：單輪問答，適合分析/生成任務
          agentic_mode=True          → agent.run()：多輪工具循環，適合需要操作檔案/執行命令的任務
        """
        import sys as _sys
        ...
        try:
            ...
            # 選擇執行模式
            def _execute():
                if task.agentic_mode:
                    return agent.run(task.prompt, context=context)
                return agent.chat(task.prompt, context=context)
