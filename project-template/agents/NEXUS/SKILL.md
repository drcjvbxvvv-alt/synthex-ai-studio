...
│
├─ 跨服務通訊，且需要保證交付
│   → Message Queue（QStash/SQS/RabbitMQ）
│
└─ 需要高吞吐量事件流（> 1000/s）
    → Kafka（成本高，複雜度高，非必要不用）
```
...
拆分微服務的條件（全部符合才考慮）：
  □ 明確識別的 Bounded Context（業務邊界清晰）
  □ 有獨立的部署和擴展需求
  □ 有足夠的工程能力維護多個服務
  □ 單體的某個模組成為瓶頸（有量測數據）

NEXUS 的預設建議：
  先單體，在單體內用清晰的模組邊界（不同目錄，不同 service/repository）
  當某個模組有明確的擴展需求時，再考慮拆出去
  「先把單體寫得像微服務，之後拆分就容易了」
```
...
**狀態**：已採用
**背景**：訂單完成後需要觸發通知、庫存扣減、分析追蹤三個動作。
直接呼叫導致 OrderService 和三個服務強耦合。

**決策**：採用內部事件總線（EventEmitter），發佈 order.completed 事件。

**理由**：
- 解耦：OrderService 不知道有哪些消費者
- 可擴展：新增消費者不修改 OrderService
- 輕量：不需要外部服務（同一個 Node.js 進程）

**後果**：
- 好：服務解耦，易於測試，易於新增消費者
- 壞：事件可能丟失（進程崩潰），需要 Event Store 確保可靠性
- 中性：需要維護事件類型定義
```
